...
void play_math_alas() {
    ...
    // Seed the random number generatorp
    srand(time(NULL));
...
void boss_battle() {
    int player_hp = 100;
    int boss_hp = 200;
    int correct_answer, user_answer, damage;
    
    // Instructions
    printf("Welcome to the Boss Battle!\n");
    printf("You need to solve math problems to deal damage to the boss.\n");

    while (player_hp > 0 && boss_hp > 0) {
        // Generate a math question
        int num1 = rand() % 20 + 1;
        int num2 = rand() % 20 + 1;
        char operator = (rand() % 2) ? '+' : '-';
        // Addition and Subtraction questions to solve
        if (operator == '+') {
            correct_answer = num1 + num2;
            printf("Solve this problem to deal damage to the boss: %d + %d\n", num1, num2);
        } else {
            correct_answer = num1 - num2;
            printf("Solve this problem to deal damage to the boss: %d - %d\n", num1, num2);
        }
        ...
        if (user_answer == correct_answer) {
            damage = rand() % 21 + 10; // Random damage between 10 and 30
            boss_hp -= damage;
            printf("Correct! You dealt %d damage to the boss.\n", damage);
        } else {
            printf("Wrong! The correct answer was %d. You dealt no damage.\n", correct_answer);
            damage = rand() % 21 + 5; // Boss attacks back with random damage between 5 and 25
            player_hp -= damage;
            printf("The boss attacks back! You took %d damage.\n", damage);
        }
    ...
    if (boss_hp <= 0) {
        ...
        printf("Game Over! The boss has defeated you!\n");
    }
}
...
int main() {
    ...
    do {
        ...
        if (ans == 1) {
            ...
            boss_battle(); // Prompt to face the boss after Math-alas
        } else if (ans == 2) {
            printf("\nWelcome to Pac-Man!\n");
            ...
            boss_battle(); // Call the Boss Battle function
        } else if (ans == 5) {
            printf("\nVery well, User: %s! I hope you enjoyed your stay.\n", username);
        } else {
            printf("\nInvalid input. Please enter 1 to play Math-alas, 2 to play Pac-Man, 3 to play Snake and Ladders, 4 to face the boss, or 5 to exit.\n");
        }
    } while (ans != 5); // Repeat until the user chooses to exit
